...
public abstract class AbstractGameState {
   ...
   */
  public static final int HIGH_SCORE_COUNT = 0;
   ...
   * @note Note that the {@link thrust.entities.in_game.Spaceship} class should
   *       be the actual owner of this data; this is just a convenience method.
   */
  // @ ensures 0 <= \result;
   ...
   * high_scores()[i].score() >= high_scores()[j].score()); @ invariant (* High
   * scores are ordered from high to low. *); @
   */

  /*
   * @ initially (* There is a fixed initial set of high scores. *); @ initially
   ...
   */
  /*
   * @ ensures new_high_score(the_new_high_score) ==> @ (\exists int i; 0 <= i &
   * i < HIGH_SCORE_COUNT; @ high_score(i).equals(the_new_high_score)); @
   */
