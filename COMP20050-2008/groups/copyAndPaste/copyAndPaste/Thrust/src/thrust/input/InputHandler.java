...
public class InputHandler {
  ...
  public void process(char the_keyboard_input) {
    ...
    switch (the_keyboard_input)  {  
      case DISPLAY_HIGH_SCORES:  inputType = new DisplayHighScores(); break;
      case TOGGLE_MUSIC_OR_EFFECTS: inputType = new ToggleMusicOrEffects(); break;
      case STOP_GAME: gameOn = false; inputType = new StopGame(); break;
      case FIRE_GUN:inputType = new FireGun(); break;
      case TURN_LEFT: inputType = new TurnLeft(); break;
      case TURN_RIGHT: inputType = new TurnRight(); break;
      case USE_ENGINE: inputType = new UseEngine(); break;
      default: /**do nothing*/
    }
    //char [space] is overloaded and has two associated behaviours 
    //if the game is not on, start the game when [space] pressed
    if (the_keyboard_input == ' ' /**START_GAME*/ &&  !gameOn) {
      inputType = new StartGame();
        gameOn = true;
    }
    //otherwise if the game is not on use shield when [space] pressed
    else if (the_keyboard_input == ' ' /**USE_SHIELD*/ &&  gameOn) {
