...
import java.util.Vector;
	...
	public class GUI extends JFrame implements ActionListener{
		...
		JButton preB, cusB, adminB, backBP, backBC, backBA, makeBP, makeBC, addBC, remBC, applyBA;
		...
		String[] ingredsList = { "None", "Vodka", "Whiskey", "Malibu", "Jagermeister", "Absinthe", "Gin", "Pineapple Juice", "Red Bull", "Coke", "Milk", "Lime", "Grenadine", "Orange Juice", "Cranberry Juice" };
		...
		public void initCocktails() {
			coke = new Beverage("Coke", 100, false, 1);
			vodka = new Beverage("Vodka", 300, true, 2);
			cocktails = new Cocktail[1];
			cocktails[0] = new Cocktail("Vodka + Coke");
			cocktails[0].addDrink(coke);
			cocktails[0].addDrink(vodka);
		...
		public void actionPerformed(ActionEvent aE) {
			...
			if(src == preB) {
				...
				if((String)tempBox.getSelectedItem() == "None" || cost1.getText() == ""){
					
				}
				else{
					tempCost = Double.valueOf(cost1.getText()).doubleValue();
					tempCost2 = (int)(tempCost * 100);
					bev1 = new Beverage((String)tempBox.getSelectedItem(), tempCost2, alc1RB.isSelected(), 1);
					bevVec.add(bev1);
				}
				
				tempBox = (JComboBox)drink2;
				if((String)tempBox.getSelectedItem() == "None" || cost2.getText() == ""){
					
				}
				else{
					tempCost = Double.valueOf(cost2.getText()).doubleValue();
					tempCost2 = (int)(tempCost * 100);
					bev2 = new Beverage((String)tempBox.getSelectedItem(), tempCost2, alc2RB.isSelected(), 2);
					bevVec.add(bev2);
				}
				
				tempBox = (JComboBox)drink3;
				if((String)tempBox.getSelectedItem() == "None" || cost3.getText() == ""){
					
				}
				else{
					tempCost = Double.valueOf(cost3.getText()).doubleValue();
					tempCost2 = (int)(tempCost * 100);
					bev3 = new Beverage((String)tempBox.getSelectedItem(), tempCost2, alc3RB.isSelected(), 3);
					bevVec.add(bev3);
				}
				setAvail();
			}
